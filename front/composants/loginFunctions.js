...
import fire from '../composants/fire';
...
class LogUser extends Component {
  ...
  login(e) {
    e.preventDefault();
    fire.auth().signInWithEmailAndPassword(this.state.email, this.state.password).then((u)=>{ history.push('/home')
    }).catch((error) => {
        console.log(error);
      });
  }
